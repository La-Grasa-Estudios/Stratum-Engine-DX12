...
public:
	...
	[[nodiscard]] Expected<bool> readFrame(Frame& frame, int decindex = 0) noexcept
	{
		Packet packet;
		...
		for (;;)
		{
			...
			if (packet.native()->stream_index == std::get<0>(vStream_)->index)
			{
				...
				auto resExp = dec->decode(packet, frame, decindex);
...
private:
	...
	std::binary_semaphore mSemaphore = std::binary_semaphore(1);
